...
fn main() {
    ...
    let herbs = "ABCDEFGHIJKLMNOPQR".chars().collect::<Vec<char>>();
    ...
    println!("hadslfhkasdlfkjasdf;lasjkdf;alsdfjk");
    for ord in order.iter().permutations(herbs.len()) {
        for v in iproduct!(
            &vec![start_pos],
            ...
            for valt in iproduct!(
                &herb_positions[*ord[10]],
                &herb_positions[*ord[11]],
                &herb_positions[*ord[12]],
                &herb_positions[*ord[13]],
                &herb_positions[*ord[14]],
                &herb_positions[*ord[15]],
                &herb_positions[*ord[16]],
                &herb_positions[*ord[17]],
                &vec![start_pos],
            ) {
                let targets = vec![
